...
const DinamicIsland = ({ textValue }) => {
  ...
  const navFunctionStaeClass = () => {
    if (animClick) return `${clas.nav} ${clas.navActive}`;
    if (!animClick) return `${clas.nav} ${clas.navActive2}`;
  ...
  return (
    <nav
      ...
      className={`${navFunctionStaeClass()} black1-bg`}>
      <button
        className={
          animClick
